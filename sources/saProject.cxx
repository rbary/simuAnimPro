...
saProject::saProject(QString title, QSize size, int debug)
  : wlCore(debug)
{
  ...
  ball->SetPosition(PX1, PY1, PZ1);
  ball->SetVelocity(VX1, VY1, VZ1);
  ball->SetReaction(PURE_KINEMATIC);
  ball->SetAttenuationCoefficientForPureKinematicReaction(k1);
  ball->Reset();
  this->balls << ball;
  ball = new wlAnimatedSphere(debug, NULL, "sphere.off");
  ball->SetPosition(PX2, PY2, PZ2);
  ball->SetVelocity(VX2, VY2, VZ2);
  ball->SetReaction(PURE_KINEMATIC);
  ball->SetAttenuationCoefficientForPureKinematicReaction(k1);
  ball->Reset();
  this->balls << ball;
